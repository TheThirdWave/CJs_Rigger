...
class SingleChainIK(maya_base_module.MayaBaseModule):
    ...
    def createControlRig(self):
        ...
        if self.prefix == 'R':
            ...
            sao_string = 'zup'
        python_utils.setOrientJoint(start_ik_joint, 'yzx', sao_string)
        python_utils.setOrientJoint(end_ik_joint, 'none', sao_string)
        ...
        base_placement = cmds.group(name='{0}_{1}_end_PLC_GRP'.format(self.prefix, self.name), parent=controls_group, empty=True)
        cmds.matchTransform(base_placement, base_control)
        cmds.parent(base_control, base_placement)
        cmds.matchTransform(base_placement, self.end_joint)
        ...
        cmds.parent(start_locator, joints_group)
        ...
        cmds.parent(base_end_locator, joints_group)
        ...
        cmds.connectAttr('{0}.distance'.format(baseDistNode), '{0}.floatA'.format(multStartLen))
        ...
        # double-scaling.
        divScaleLen = cmds.shadingNode('floatMath', name='{0}_{1}_{2}_{3}'.format(prefix, component_name, joint_name, 'ik_1_LEN_CMATH'), asUtility=True)
        ...
        cmds.connectAttr('{0}.distance'.format(baseDistNode), '{0}.floatB'.format(divScaleLen))
